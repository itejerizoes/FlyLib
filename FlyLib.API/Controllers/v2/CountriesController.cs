...
namespace FlyLib.API.Controllers.v2
{
    ...
    public class CountriesController : ControllerBase
    {
        ...
        /// <summary>
        /// Obtener todos los países (v2: incluye continente).
        /// </summary>
        [Authorize(Roles = $"{AppRoles.Admin},{AppRoles.User}")]
        [HttpGet]
        [ProducesResponseType(typeof(IEnumerable<CountryResponseV2>), 200)]
        public async Task<IActionResult> GetAll()
        {
            var result = await _mediator.Send(new GetAllCountriesQuery());
            return Ok(_mapper.Map<IEnumerable<CountryResponseV2>>(result));
        }

        /// <summary>
        /// Obtener un país por Id (v2).
        /// </summary>
        [Authorize(Roles = $"{AppRoles.Admin},{AppRoles.User}")]
        [HttpGet("{id:int}")]
        [ProducesResponseType(typeof(CountryResponseV2), 200)]
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            return Ok(_mapper.Map<CountryResponseV2>(result));
        }

        /// <summary>
        /// Crear un país (v2: requiere continente).
        /// </summary>
        [Authorize(Roles = AppRoles.Admin)]
        [HttpPost]
        [ProducesResponseType(typeof(CountryResponseV2), 201)]
        ...
        /// <summary>
        /// Registrar un nuevo país (v2).
        /// </summary>
        [Authorize(Roles = AppRoles.Admin)]
        [HttpPost]
        [ProducesResponseType(typeof(CountryResponseV1), 201)]
        [ProducesResponseType(400)]
        public async Task<IActionResult> Create([FromBody] CreateCountryRequestV2 request)
        {
            var cmd = _mapper.Map<CreateCountryCommand>(request);
            var created = await _mediator.Send(cmd);

            var response = _mapper.Map<CountryResponseV2>(created);
            return CreatedAtAction(nameof(GetById), new { id = created.CountryId }, response);
        }

        /// <summary>
        /// Actualizar un país (v2).
        /// </summary>
        [Authorize(Roles = AppRoles.Admin)]
        [HttpPut("{id:int}")]
        [ProducesResponseType(204)]
        [ProducesResponseType(400)]
        public async Task<IActionResult> Update(int id, [FromBody] UpdateCountryRequestV2 request)
        {
